...
export default function ActivityHeatmap({
  ...
  const { startDate, endDate, days } = useMemo(() => {
    ...
    let length = 84; // default 3 months
    if (range === '1m') length = 28;
    if (range === '1w') length = 7;
  ...
  return (
    <div className="w-full border-l-4 border-emerald-500 px-4">
      <h5 className="font-semibold mb-2">Activity Tracker</h5>
      <div className="flex items-center align-bottom gap-3 mb-3">
        ...
        <div className="text-xs text-gray-500 dark:text-gray-300w">
              ...
              {daysArr.map((d, i) => {
                ...
                return (
                  <div
                    key={i}
                    title={title}
                    className={`w-6 h-6 rounded-sm ${color} ${border} cursor-default`}
                  />
                );
              })}
            </div>
